...
# Set page config
st.set_page_config(page_title="Clinical Note Classifier", page_icon="🏥", layout="wide")
...
def predict(text, model, tokenizer):
    try:
        ...
        device = next(
            model.parameters()
        ).device  # Accesses the device attribute of that parameter, which will be either
        inputs = {
            k: v.to(device) for k, v in inputs.items()
        }  # iterates through the inputs dictionary
        ...
        for prob, idx in zip(top3_prob[0], top3_indices[0]):
            label = model.config.id2label[
                str(idx.item())
            ]  # Convert idx to string for dictionary lookup
...
def load_model_and_tokenizer():
    try:
        ...
        model = AutoModelForSequenceClassification.from_pretrained(
            model_path, num_labels=len(id2label), id2label=id2label
        )
    ...
    if text_input.strip() == "":
        ...
        with st.spinner("Analyzing..."):
            try:
                ...
                st.error(
                    "Please try again with different text or contact support if the issue persists."
                )
