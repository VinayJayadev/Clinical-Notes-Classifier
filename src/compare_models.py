...
def plot_confusion_matrix(cm, model_name, labels):
    ...
    # Log to MLflow
    mlflow.log_figure(plt.gcf(), f"confusion_matrices/{model_name}_confusion_matrix.png")
...
def plot_metrics_comparison(metrics):
    ...
    plt.bar(x - width*0.5, metrics_df['macro_avg_precision'], width, label='Macro Avg Precision')
    plt.bar(x + width*0.5, metrics_df['macro_avg_recall'], width, label='Macro Avg Recall')
...
def plot_class_performance(metrics):
    ...
    for metric in metrics:
        ...
        # Log to MLflow
        mlflow.log_figure(plt.gcf(), f"visualizations/{model_name}_class_performance.png")
...
def plot_prediction_differences(metrics):
    ...
    # Get unique labels
    labels = sorted(set(metrics[0]['true_labels']))
    ...
    # Calculate agreement
    agreement = (comparison['Logistic Regression'] == comparison['Random Forest']).mean()
    
    # Plot agreement by class
    plt.figure(figsize=(15, 6))
    agreement_by_class = comparison.groupby('True Label').apply(
        lambda x: (x['Logistic Regression'] == x['Random Forest']).mean()
...
def log_metrics_to_mlflow(metrics):
    ...
    for metric in metrics:
        ...
        mlflow.log_metric(f"{model_name}_macro_avg_precision", metric['macro_avg_precision'])
        ...
        mlflow.log_metric(f"{model_name}_weighted_avg_precision", metric['weighted_avg_precision'])
        mlflow.log_metric(f"{model_name}_weighted_avg_recall", metric['weighted_avg_recall'])
        ...
        for label, class_metric in metric['class_metrics'].items():
            mlflow.log_metric(f"{model_name}_{label}_precision", class_metric['precision'])
...
def log_artifacts_to_mlflow(metrics):
    ...
    if os.path.exists('./results/classical_ml/model_comparison.png'):
        mlflow.log_artifact('./results/classical_ml/model_comparison.png', "visualizations")
    if os.path.exists('./results/classical_ml/model_agreement.png'):
        mlflow.log_artifact('./results/classical_ml/model_agreement.png', "visualizations")
    
    # Log reports
    if os.path.exists('./results/classical_ml/model_comparison_report.json'):
        mlflow.log_artifact('./results/classical_ml/model_comparison_report.json', "reports")
    if os.path.exists('./results/classical_ml/model_comparison_report.md'):
        mlflow.log_artifact('./results/classical_ml/model_comparison_report.md', "reports")
...
def main():
    ...
    with mlflow.start_run(experiment_id=experiment_id) as run:
        ...
        mlflow.log_param("evaluation_metrics", "accuracy,precision,recall,f1,confusion_matrix")
        ...
        for model_name, model_path in models.items():
            if os.path.exists(model_path):
                ...
                mlflow.log_artifact(model_path, f"models/{model_name.lower().replace(' ', '_')}")
                
                model = ClinicalNoteClassifier()
                model.load_model(model_path)
                
                # Log model hyperparameters
                if hasattr(model.pipeline, 'named_steps') and 'classifier' in model.pipeline.named_steps:
                    classifier = model.pipeline.named_steps['classifier']
                    if hasattr(classifier, 'get_params'):
                        params = classifier.get_params()
                        for param_name, param_value in params.items():
                            mlflow.log_param(f"{model_name.lower().replace(' ', '_')}_{param_name}", param_value)
