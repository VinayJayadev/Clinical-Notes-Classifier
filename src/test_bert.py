...
def test_bert_model(model_path="./results/best_model", data_path="./data/mtsamples.csv"):
    """
    ...
    plt.tight_layout()
    plt.savefig("./results/bert_evaluation/bert_confusion_matrix.png", dpi=300, bbox_inches='tight')
    plt.close()
    ...
    if len(top_classes) > 0:
        ...
        for i, class_name in enumerate(classes[:10]):
            ax2.annotate(class_name, (precisions[i], recalls[i]), 
        ...
        if len(classes) > 10:
            ax2.text(0.02, 0.98, f'Top 10 classes labeled\n{len(classes)-10} additional classes shown as points', 
                    transform=ax2.transAxes, fontsize=10, verticalalignment='top',
                    bbox=dict(boxstyle='round,pad=0.5', facecolor='lightgray', alpha=0.8))
        
        plt.tight_layout()
        plt.savefig("./results/bert_evaluation/bert_class_performance.png", dpi=300, bbox_inches='tight')
        plt.close()
    else:
...
"""
    ...
    print("="*60)
    print(f"Results saved to: ./results/bert_evaluation/")
    print(f"- bert_test_results.json")
    print(f"- bert_classification_report.txt")
    print(f"- bert_confusion_matrix.png")
    print(f"- bert_class_performance.png")
    print(f"- bert_evaluation_report.md")
