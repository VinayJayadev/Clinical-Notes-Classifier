...
print(
    f"Data loaded. Train size: {len(raw_datasets['train'])}, Test size: {len(raw_datasets['test'])}"
)
...
class SafeCheckpointCallback(
    TrainerCallback
):  # Callbacks allow you to inject custom logic at various stages of the training loop
...
if save_model_safely(trainer, best_model_path):
    ...
    print(
        "Warning: Could not save the best model. You may need to manually save it later."
    )
